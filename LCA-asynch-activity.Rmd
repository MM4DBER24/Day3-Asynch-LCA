...
write_csv(table_data1, here("patterns_by_C.csv"))
...
> The goal of this lab activity is to provide an opportunity for you to “get your hands into” the posterior probabilities and get practice calculating the Average Posterior Probabilities (AvePP) for each class. Specifically, we are going to recreate this table from the Mplus output. One way to think of the AvePP values are as the class-specific classification errors in the model.
...
out_c4 <- readModels(here("13-response-patterns", "mplus_files"),
